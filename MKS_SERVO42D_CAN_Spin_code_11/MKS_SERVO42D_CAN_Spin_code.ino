...
#define MOTOR_CAN_ID 0x02        // CAN ID //cange ID back to 1 once done test
...
void mks_move_absolute_axis(int32_t axis, uint16_t speed, uint8_t acc) {
  ...
  uint8_t p[] = {
    0xF5,
    (uint8_t)((speed >> 8)), // speed high
    (uint8_t)(speed),        // speed low
    acc,
    (uint8_t)((axis >> 16)),
    (uint8_t)((axis >> 8)),
    (uint8_t)(axis)         // axis values
  };
  mks_send(MOTOR_CAN_ID, p, sizeof(p));
}
  ...
  if (twai_receive(&rx_msg, pdMS_TO_TICKS(10)) == ESP_OK) {
    if (rx_msg.data_length_code >= 2) {
      ...
      if (cmd == 0x31) {
        ...
        int revo = (encoder/16385)/360;
        Serial.printf("Encoder Position = %ld pulses (revo%ld)\n", encoder, revo);
      }
      // RPM feedback
